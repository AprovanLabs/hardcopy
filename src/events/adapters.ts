...
import { getWebhookInferrer } from "./types";
...
export class WebhookAdapter {
  ...
  async handle(
    ...
    const subject = this.config.subjectExtractor
      ...
      : this.inferSubject(provider, body, headers);
  ...
  private inferType(provider: string, body: unknown, headers: Record<string, string>): string {
    const inferrer = getWebhookInferrer(provider);
    if (inferrer) {
      const inferred = inferrer.inferType(body, headers);
      if (inferred) return inferred;
    }
    return `webhook.${provider}.event`;
  }

  private inferSubject(provider: string, body: unknown, headers: Record<string, string>): string | undefined {
    const inferrer = getWebhookInferrer(provider);
    if (inferrer) {
      return inferrer.inferSubject(body, headers);
    }
    return undefined;
  }
}
