...
const webhookInferrers = new Map<string, WebhookInferrer>();

export function registerWebhookInferrer(inferrer: WebhookInferrer): void {
  webhookInferrers.set(inferrer.provider, inferrer);
}

export function getWebhookInferrer(provider: string): WebhookInferrer | undefined {
  return webhookInferrers.get(provider);
}

export function listWebhookInferrers(): WebhookInferrer[] {
  return Array.from(webhookInferrers.values());
}
